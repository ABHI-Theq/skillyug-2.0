...
export  const checkoutHandler = async (amount,courseId) => {
        try {
            const { data: { key } } = await axios.get("https://skillyug-2-0-backend.onrender.com/api/getkey");

            const { data: { order } } = await axios.post("https://skillyug-2-0-backend.onrender.com/api/checkout", {
                amount,
                courseId
            });
            console.log(order);
